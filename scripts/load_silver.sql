...
WHERE flag_last =1 
    ...
    SELECT 
        REPLACE(cid,'-',''),
        CASE
			WHEN UPPER(TRIM(REGEXP_REPLACE(cntry, '\\s+', ''))) = 'DE' THEN 'Germany'
			WHEN UPPER(TRIM(REGEXP_REPLACE(cntry, '\\s+', ''))) IN ('US','USA','UNITEDSTATES') THEN 'United States'
			WHEN cntry IS NULL OR TRIM(REGEXP_REPLACE(cntry, '\\s+', '')) = '' THEN 'Unknown'
